...
WILD setup has three practical stages: prepare the hardware, record data, and run post-processing. Firmware flashing is the final step of hardware setup.

<div class="wild-grid two">
  <div class="wild-card">
    <h3>1. Hardware setup</h3>
    <p>Check the device, connectors, microSD card, battery, probe or sensor cabling, then flash the bootloader or stage the application image as the last hardware-preparation step.</p>
  </div>
  <div class="wild-card">
...
!!! tip "Recommended first path"
    If this is your first device, work through Hardware Setup, Data Acquisition, and Data Analysis in order. Most acquisition failures are easier to diagnose when the SD card, battery, firmware image, and boot state are known.
