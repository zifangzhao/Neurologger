(() => {
  const initWildZoom = () => {
    const stage = document.querySelector(".wild-zoom-stage");
    if (!stage || stage.dataset.wildZoomReady === "true") return;
    ...
    stage.dataset.wildZoomReady = "true";
    ...
    updateZoom();
    window.setTimeout(updateZoom, 120);
    window.addEventListener("scroll", updateZoom, { passive: true });
    window.addEventListener("resize", updateZoom);
    window.addEventListener("scrollend", updateZoom);
    reducedMotion.addEventListener?.("change", updateZoom);

    if ("IntersectionObserver" in window) {
      const observer = new IntersectionObserver(updateZoom, {
  ...
  if (window.document$ && typeof window.document$.subscribe === "function") {
    window.document$.subscribe(initWildZoom);
  } else if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initWildZoom, { once: true });
  } else {
    initWildZoom();
  }
})();
